...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 테이블 위젯 컬럼 크기 조절
        table.setColumnWidth(0, self.width()*4/30)
        table.setColumnWidth(1, self.width()*1.4/30)
        table.setColumnWidth(2, self.width()*1/30)
        table.setColumnWidth(3, self.width()*2.4/30)
    ...
    def salesListWidget_ItemClicked(self):
        ...
        for item in range(len(receipt_value)):
            if len(receipt_value[item][1]) <= 2:
                ...
                self.ui.salesText.appendPlainText(f'''{receipt_value[item][1]}\t\t{format((receipt_value[item][2]), ',')}\t\t\t{format((receipt_value[item][3]), ',')}\t\t{format((receipt_value[item][4]), ',')}\n''')
            elif len(receipt_value[item][1]) <= 7:
                self.ui.salesText.appendPlainText(f'''{receipt_value[item][1]}\t\t\t{format((receipt_value[item][2]), ',')}\t\t\t{format((receipt_value[item][3]), ',')}\t\t{format((receipt_value[item][4]), ',')}\n''')
            elif len(receipt_value[item][1]) > 7:
                self.ui.salesText.appendPlainText(f'''{receipt_value[item][1]}\t\t{format((receipt_value[item][2]), ',')}\t\t\t{format((receipt_value[item][3]), ',')}\t\t{format((receipt_value[item][4]), ',')}\n''')
        # 하단 총액 폰트 사이즈 변경            
    ...
    def make_Add_Button(self, rowPlaceNum, colPlaceNum):
        ...
        # 버튼 사이즈 조절 부분
        btn_Add.setMinimumSize(QSize(100, 60))
        btn_Add.setMaximumSize(QSize(100, 60))
    ...
    def make_Sub_Button(self, rowPlaceNum, colPlaceNum):
        ...
        # 버튼 사이즈 조절 부분
        btn_Sub.setMinimumSize(QSize(100, 60))
        btn_Sub.setMaximumSize(QSize(100, 60))
        # 버튼 내 텍스트 변경 부분
        btn_Sub.setText('제거')
    ...
    def make_Del_Button(self, rowPlaceNum, colPlaceNum):
        ...
        # 버튼 사이즈 조절 부분
        btn_Del.setMinimumSize(QSize(60, 60))
        btn_Del.setMaximumSize(QSize(60, 60))
        # 버튼 내 텍스트 변경 부분
        btn_Del.setText('X')
