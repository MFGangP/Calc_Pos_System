...
import 'package:possystem/View/Component/homeview_menu_button_view.dart';
import 'package:possystem/View/Component/homeview_oreder_button_view.dart';
...
class _PosHomeViewState extends State<PosHomeView> {
                                      ...
                                      GestureDetector(
                                        ...
                                        child: Padding(
                                          ...
                                          child: HomeViewMenuButton(
                                            ...
                                            menuPrice: int.parse(productList[j]['prdPrice']!),
                                            onMenuCountChanged: (String menuName, int menuPrice, int menuCount) {
  ...
  void addOrUpdateMenuRow(String menuName, int menuPrice, int menuCount, bool countingBool) {
    setState(() {
      // 항목이 이미 존재하는지 확인
      final existingRowIndex = tableContentList.indexWhere((row) {
        ...
        final currentCount = int.parse(menuCountText.data!);
          if (countingBool == true){
            // 수량이 변경된 경우에만 업데이트
            final updatedRow = buildDataRow(menuName, menuPrice, currentCount + 1);
          ...
          // 항목이 없으면 새로 추가
          tableContentList.add(
            buildDataRow(menuName, menuPrice, menuCount)
          );
        }
      }
    );
